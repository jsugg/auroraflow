  ...
  defineConfig({
    test: {
      coverage: {
        ...
        // node coverage like the browser-only DOM snapshot path.
        exclude: ['src/playwright.ts'],
        thresholds: {
          // Global aggregate floor: erosion guard only. Risk lives in the
          // per-file floors below, not in this aggregate.
          statements: 78,
          branches: 68,
          functions: 85,
          lines: 78,
          ...
          // Structured candidate model: locator resolution and the legacy
          // string read path are correctness-critical, so they carry a floor
          // below measured coverage to guard against erosion.
          ...
          // Risk-weighted floors for high-risk library surfaces that a global
          // aggregate would otherwise hide. Floors sit below measured coverage
          // so a failure means real erosion, not noise.
          ...
          // Exemption expiry: revisit when structured candidates move the browser
          // logic into node-testable units.
