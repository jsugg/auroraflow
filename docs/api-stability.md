...
- **Artifact schemas** (`schemas/*.json`, shipped in the npm package): self-healing failure events, DOM snapshots, versioned selector registry records, candidate histories, pending promotions, flakiness summaries, SLO dashboards, alert evaluations, and observability trend points are frozen by v1 compatibility fixtures. Versioned payloads carry `schemaVersion`/`artifactVersion`; older contracts are versioned by published schema and fixture path. Schema changes follow the same semver discipline as `advanced` exports and are validated by `npm run schemas:check`. See [Artifact schemas](./operations/artifact-schemas.md).
...
| `RedisCompareAndSetJsonFieldOptions`         | type    | advanced |
...
| `SelectorStoreCompareAndSetJsonFieldOptions` | type    | advanced |
| `SelectorStoreCompareAndSetOptions`          | type    | advanced |
| `SelectorStoreCompareAndSetResult`           | type    | advanced |
| `SelectorStoreJsonPrimitive`                 | type    | advanced |
...
| `PromotionStatusConflictError`                   | runtime | advanced     |
| `SelfHealingPromotionWorkflow`                   | runtime | advanced     |
| `ApprovePromotionInput`                          | type    | advanced     |
| `PromotionAuthorizationError`                    | runtime | advanced     |
| `createPromotionAuthorizationPolicy`             | runtime | advanced     |
| `CreatePromotionAuthorizationPolicyOptions`      | type    | advanced     |
| `PromotionAuthorizationAction`                   | type    | advanced     |
| `PromotionAuthorizationDecision`                 | type    | advanced     |
| `PromotionAuthorizationEvidence`                 | type    | advanced     |
| `PromotionAuthorizationInput`                    | type    | advanced     |
| `PromotionAuthorizationMode`                     | type    | advanced     |
| `PromotionAuthorizationPolicy`                   | type    | advanced     |
...
| `DEFAULT_PROMOTION_AUDIT_RETENTION_SECONDS`      | runtime | advanced     |
| `MAX_PROMOTION_AUDIT_RETENTION_SECONDS`          | runtime | advanced     |
