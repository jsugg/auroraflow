...
| `AUR-IMPL-009` | Add API stability tiers | 1 | P1 | governance | Complete | `008`, `031` | API governance | M | Phase 0 defaults | none; public npm library resolved | Every root export classified and contract-tested. |
...
| `AUR-IMPL-020` | Implement structured candidate model end-to-end | 2 | P1 | refactor | Complete | `002`, `008`, `022`, `040` | self-healing/runtime | L | `003`, `009` | schema compatibility | New runtime path uses discriminated locator model and legacy read. |
| `AUR-IMPL-021` | Add `AuroraFlowContext` runtime injection | 2 | P1 | architecture | Complete | `011`, `006`, `010` | runtime architecture | L | `013`, `009` | none | Two contexts run isolated in one process. |
| `AUR-IMPL-022` | Add internal page-action pipeline | 2 | P1 | refactor | Complete | `010`, `011`, `027`, `025`, `040` | runtime architecture | L | `021`, `020` | none | Click/type delegate through the internal pipeline without public behavior regression. |
...
| `AUR-IMPL-024` | Add run-level self-healing budget | 2 | P1 | reliability | Complete | `013`, `028`, `007` | runtime/reliability | M | `021` or run-state seam | none; baseline-first warning-only resolved | Failure storm bounded and warned once. |
...
- **Implementation outline:** Add context interface/default context; thread optional context through `PageObjectBase` and explicit `PageFactory` providers; use default adapters for compatibility; add two-context test.
...
- **Completion evidence:** `docs/architecture/locator-first-api.md` design/API review package and docs contract coverage; type tests deferred because no overload prototype ships.
- **Journal update required:** Yes
...
| Current task | `AUR-IMPL-027` locator-first API ergonomics design complete |
| Branch | `main` |
| Latest commit | `4440ba7` (`Merge pull request #105 from jsugg/qe/runtime-isolation-aur-qe-112-114`); current `AUR-IMPL-022`, `AUR-IMPL-024`, `AUR-IMPL-025`, `AUR-IMPL-026`, and `AUR-IMPL-027` working tree changes pending commit |
| Working tree status | Internal page-action pipeline, run-level budget source/tests/docs, promotion authorization/audit retention, and locator-first API-review docs pending commit |
| Blocked decision gates | None; `AUR-DEC-001`-`AUR-DEC-013` resolved by operator on 2026-06-10 |
| Last validation run | 2026-06-25 `npm run verify` passed (format, lint, typecheck, unit 55 files / 358 tests, contracts 19 / 93, integration 2 / 16, schemas, ShellCheck, workflow lint); `npm run build` and `npm run test:coverage:critical` also passed. |
| Next recommended action | Submit the locator-first design for API review before adding overload prototypes; otherwise continue Phase 3 with `AUR-IMPL-028` or reconcile `AUR-IMPL-023` lifecycle fixture status. |
...
| 2026-06-10 | Codex | `AUR-IMPL-003` | Added guarded locator parser regressions for apostrophes, quote escapes, role/name, label, CSS, and DOM-emitted candidate compatibility; added temporary string-DSL parser hotfix linked to `AUR-IMPL-020` | `src/framework/selfHealing/guardedValidation.ts`, `tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts`, `tests/suites/unit/framework/selfHealing/domCandidateExtraction.spec.ts`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts` before hotfix; `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts`; `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts tests/suites/unit/framework/selfHealing/domCandidateExtraction.spec.ts`; `rtk npm run typecheck` | Initial regression run failed on apostrophe/escaped CSS as expected; hotfix passed 38 files / 190 tests, final targeted suite passed 38 files / 191 tests; typecheck passed | Next task: `AUR-IMPL-005` |
| 2026-06-10 | Codex | `AUR-IMPL-005`, `AUR-IMPL-006` | Implemented atomic candidate-history writes via optional `SelectorStore.atomicJsonMerge` backed by a Redis Lua `EVAL` (no process-local locks; missing capability fails writes explicitly); aligned TTL contract so exported default and hard cap are both 2,592,000 seconds (30 days) per `AUR-DEC-005`; added TTL default/custom/clamp/invalid unit tests, a 128-observation parallel-write unit test, a 120-observation Redis concurrency integration test, schema `$comment`/example, and docs updates; exported `MAX_SELECTOR_CANDIDATE_HISTORY_TTL_SECONDS` | `src/framework/selfHealing/historyRepository.ts`, `src/utils/redisClient.ts`, `src/data/selectors/selectorRegistry.ts`, `src/data/selectors/redisSelectorStore.ts`, `src/index.ts`, `schemas/selector-candidate-history.schema.json`, `tests/suites/unit/framework/selfHealing/historyRepository.spec.ts`, `tests/suites/unit/framework/selfHealing/{promotionWorkflow,registryRuntime}.spec.ts`, `tests/suites/unit/framework/packageSurface/packageSurface.spec.ts`, `tests/suites/integration/framework/data/redisIntegration.spec.ts`, `docs/architecture/{data-layer,self-healing}.md`, `docs/configuration.md`, `docs/operations/artifact-schemas.md` | Session interrupted before validation/journal; see 2026-06-11 entry | Implementation complete in working tree; validation pending | Validate and journal `AUR-IMPL-005`/`006` |
| 2026-06-11 | Claude | `AUR-IMPL-005`, `AUR-IMPL-006` | Verified interrupted Codex implementation against task cards and `AUR-DEC-005`; fixed Prettier formatting in five touched files; ran all validation gates and recorded completion evidence | `docs/configuration.md`, `src/utils/redisClient.ts`, `tests/suites/unit/framework/selfHealing/{historyRepository,promotionWorkflow,registryRuntime}.spec.ts` (formatting only), `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/historyRepository.spec.ts ...` (all touched specs); `rtk npm run test:integration -- --run tests/suites/integration/framework/data/redisIntegration.spec.ts`; `rtk npm run schemas:check`; `rtk npm run typecheck`; `rtk npm run lint`; `rtk npm run format:check` | Unit script ran 39 files / 199 tests passed; integration script ran 16 files / 61 tests passed including 120-observation Redis concurrency counters with zero lost increments; schemas compiled (10), typecheck/lint clean; format:check passed after targeted Prettier write | Next task: `AUR-IMPL-007` |
| 2026-06-11 | Claude | `AUR-IMPL-007` | Added `SELF_HEAL_*` config diagnostics: `resolveSelfHealingConfigWithDiagnostics()` returns `{ config, diagnostics, strict }` without logging; `resolveSelfHealingConfig()` keeps its signature, warns one diagnostic per invalid value by default (injectable logger), and throws `SelfHealingConfigError` when `AURORAFLOW_CONFIG_STRICT=true` (or `strict` option). Diagnostics name variable/expected format/applied fallback and never echo received values (secret-leak test included). Added `describeEffectiveSelfHealingConfig()` JSON-safe snapshot. Numeric parsing tightened from lenient `parseInt`/`parseFloat` to strict `Number()` (non-integer or garbage-suffixed values now fall back to defaults with a diagnostic); clamps above hard maxima now report `clamped`. `SELF_HEAL_PROMOTION_MODE` documented as reserved for future enforcement under `AUR-IMPL-025`. Exported new API from package root | `src/framework/selfHealing/config.ts`, `src/index.ts`, `tests/suites/unit/framework/selfHealing/config.spec.ts`, `tests/suites/unit/framework/packageSurface/packageSurface.spec.ts`, `docs/configuration.md`, `docs/architecture/self-healing.md`, `README.md`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/config.spec.ts tests/suites/unit/framework/packageSurface/packageSurface.spec.ts`; `rtk npm run typecheck`; `rtk npm run lint`; `rtk npm run format:check`; `rtk npm run test:integration -- --run tests/suites/contracts/docs/documentationSurface.contract.spec.ts` | Unit script ran 39 files / 212 tests passed (13 new diagnostics/strict/effective-config tests); docs contract 16 files / 61 tests passed; typecheck/lint/format clean | Next task: `AUR-IMPL-009` |
| 2026-06-11 | Claude | Phase 0 review (`AUR-IMPL-001`-`008`) | Re-reviewed PR-00A through PR-00E implementations against task cards and acceptance criteria; added the missing legacy-store explicit-failure test for `SelectorStore.atomicJsonMerge`; corrected the `AUR-IMPL-006` targeted command in the Phase 0 validation baseline to the real `historyRepository.spec.ts`; committed the Phase 0 working tree as task-scoped commits on `phase-0-credibility-hardening` | `tests/suites/unit/framework/selfHealing/historyRepository.spec.ts`, `docs/architecture/phase-0-validation-baseline.md`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npm run schemas:check`; `rtk npm run verify` | Schemas compiled (10); verify passed: format/lint/typecheck clean, unit 39 files / 213 tests, integration 16 files / 61 tests, shellcheck and workflows lint clean | Open the Phase 0 PR, follow CI to green, merge, then `AUR-IMPL-009` |
| 2026-06-11 | Claude | `AUR-IMPL-004` follow-up | PR #80 CI: Observability Full Stack Smoke failed because the retry-failure Prometheus rule now references `auroraflow_test_attempts_total`, the live-export assert derives required series from rule expressions, and the CI smoke workload never emitted that counter. Fixed by emitting succeeded/failed `testAttemptsTotal` counters in `recordRepresentativeSmokeMetrics`, matching the `auroraflow.test_attempt.status` attribute convention from `reportTelemetry.ts` | `scripts/observability-ci-smoke.ts`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk gh run view --job <full-stack-smoke> --log-failed`; `rtk npm run lint`; `rtk npm run typecheck`; prettier check on the script | Root cause confirmed from job log (`missing metric series: auroraflow_test_attempts_total`); lint/typecheck/format clean; full-stack smoke re-validated in PR CI | Watch PR #80 checks; merge when green |
...
| 2026-06-18 | Codex | `AUR-IMPL-020` | Implemented the discriminated `CandidateLocator` model end-to-end: structured locator producers for heuristic/DOM candidates, candidate scoring propagation, guarded validation resolving structured candidates without parsing display strings, schema-versioned JSON shape, legacy string read converter, root export classification, and quote/legacy/schema regression coverage. Hardened structured regex artifact parsing and contained malformed structured resolver errors to a single guarded-validation candidate. | `src/framework/selfHealing/{candidateLocator,candidateTypes,domCandidateExtraction,suggestionEngine,candidateScoring,guardedValidation,types}.ts`, `src/index.ts`, `schemas/_shared.schema.json`, `tests/suites/unit/framework/selfHealing/{candidateLocator,guardedValidation,domCandidateExtraction,artifactSchema,artifactJsonSchemas,candidateScoring}.spec.ts`, `docs/api-stability.md`, `docs/development.md`, `docs/operations/artifact-schemas.md`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/candidateLocator.spec.ts tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts tests/suites/unit/framework/selfHealing/artifactJsonSchemas.spec.ts tests/suites/unit/framework/selfHealing/artifactSchema.spec.ts tests/suites/unit/framework/selfHealing/domCandidateExtraction.spec.ts tests/suites/unit/framework/selfHealing/candidateScoring.spec.ts`; `rtk npm run schemas:check`; `rtk npm run typecheck`; `rtk npm run lint`; `rtk npm run format:check`; `rtk npm run test:integration -- --run tests/suites/contracts/package/packageSurface.contract.spec.ts`; `rtk npm run verify` | Targeted gates passed; full verify passed after updating the taxonomy contract to the now-implemented risk-weighted coverage wording. Unit runner executed 50 files / 314 tests plus 1 expected fail; contracts passed 19 files / 91 tests; integration passed 2 files / 16 tests; schemas compiled 10. | Continue with `AUR-IMPL-021` |
| 2026-06-18 | Claude | `AUR-IMPL-021` | Added the `AuroraFlowContext` runtime dependency container (`src/framework/runtime/auroraFlowContext.ts`) with env-backed defaults and per-port overrides for logger, correlation, telemetry, self-healing config, registry runtime, artifact privacy policy, and an injectable clock. Threaded an optional `context` parameter through the `PageObjectBase` constructor (default `createAuroraFlowContext()`) and initially through `PageFactory`. Migrated the facade internals (`safeAction`/`safeActionWithTelemetry` telemetry, self-healing config, `resolveRegistryRuntime`/`resolveArtifactPrivacyPolicy` seams, screenshot timestamp, duration clock) off direct `process.env`/`getTelemetry()` reads and onto `this.context`. Test-first: authored the two-context isolation spec before migrating internals and confirmed it failed on telemetry/config bleed, then passed after the migration. Correction recorded on 2026-06-19: implicit factory constructor injection was not compatible with domain-owned second constructor arguments; `PageFactory.getPage()` is page-only and context-aware construction uses explicit providers. Scope boundary: downstream self-healing subsystems (suggestion engine, guarded validation, failure capture, registry persistence) still emit to the telemetry singleton; threading context through them is `AUR-IMPL-022`. | `src/framework/runtime/auroraFlowContext.ts`, `src/pageObjects/pageObjectBase.ts`, `src/helpers/pageFactory.ts`, `src/index.ts`, `docs/api-stability.md`, `tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npm run test:unit -- --run tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts` (red pre-migration, then green); `rtk npm run typecheck`; `rtk npm run lint`; `rtk npm run format:check` (with targeted `prettier --write`); `rtk npm run test:unit` (full); `rtk npm run test:integration -- --run tests/suites/contracts/package/packageSurface.contract.spec.ts` | Isolation spec demonstrated red (`expected [] to have a length of 1`) then green after migration; typecheck/lint/format clean; full unit suite 51 files / 316 tests + 1 expected fail; package-surface contract 3 files / 23 tests passed (5 new experimental exports classified). Later typecheck caught the `ExamplePage(page, url?)` incompatibility; see 2026-06-19 correction. | Continue with `AUR-IMPL-022` |
| 2026-06-19 | Codex | `AUR-IMPL-021` remediation | Corrected the runtime-context factory seam after `typecheck` proved that implicit `PageFactory` context injection broke `ExamplePage(page, url?)` and the stable `PageObjectConstructor` contract. `PageObjectConstructor` is page-only again, `PageFactory.getPage()` constructs with the Playwright `Page` only, and new `PageObjectProvider` / `registerPageProvider()` provide the explicit opt-in seam for context-aware or domain-argument construction. Documented the decision in `docs/ARCHITECTURE_IMPROVEMENT_PLAN.md`: page-object constructor positions are domain-owned, while runtime infrastructure binds through explicit providers/fixtures. | `src/helpers/pageFactory.ts`, `src/index.ts`, `tests/suites/unit/framework/pageFactory/pageFactory.spec.ts`, `tests/suites/unit/framework/publicApi/publicApi.spec.ts`, `tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts`, `docs/api.md`, `docs/api-stability.md`, `docs/ARCHITECTURE_IMPROVEMENT_PLAN.md`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npm run typecheck`; `rtk npx vitest run tests/suites/unit/framework/pageFactory/pageFactory.spec.ts tests/suites/unit/framework/publicApi/publicApi.spec.ts tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts --pool=threads --no-isolate --testTimeout=30000`; `rtk npm run test:contracts -- --run tests/suites/contracts/package/packageSurface.contract.spec.ts`; `rtk npm run format:check`; `rtk npm run lint`; `rtk npm run verify` | Passed. Targeted unit specs: 3 files / 10 tests. Contracts: 19 files / 91 tests. Full verify: unit 51 files / 320 tests + 1 expected fail; contracts 19 / 91; integration 2 / 16; schemas compiled 10; ShellCheck/workflow lint passed. | Continue with `AUR-IMPL-022`; use `registerPageProvider()` or the future fixture for factory-owned context. |
| 2026-06-24 | Automation | `AUR-IMPL-022` | Added the internal `PageActionPipeline` first slice and migrated public `PageObjectBase.click()` / `type()` delegation through it. The facade still normalizes action options and uses the existing safe-action envelope, while the pipeline owns the click/type Playwright operation, success/error messages, action context handoff, and guarded retry apply callback. `PageActionError` now also sets native `Error.cause` to the original action error while preserving the existing `originalError` property, so a failed guarded retry keeps the original Playwright failure primary. The failure path now passes context-owned telemetry into suggestion generation, guarded validation, failure capture, and registry persistence while direct helper calls keep singleton fallback behavior. | `src/pageObjects/pageActionPipeline.ts`, `src/pageObjects/pageObjectBase.ts`, `src/framework/selfHealing/{suggestionEngine,guardedValidation,failureCapture,registryPersistence}.ts`, `tests/suites/unit/framework/pageObjectBase/pageActionPipeline.spec.ts`, `tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npx vitest run tests/suites/unit/framework/pageObjectBase/pageActionPipeline.spec.ts tests/suites/unit/framework/pageObjectBase/pageObjectBase.spec.ts tests/suites/unit/framework/pageObjectBase/pageObjectBaseSelfHealing.spec.ts tests/suites/unit/framework/pageObjectBase/pageObjectBaseTelemetry.spec.ts --pool=threads --no-isolate --testTimeout=30000`; `rtk npx vitest run tests/suites/unit/framework/selfHealing/suggestionEngine.spec.ts tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts tests/suites/unit/framework/selfHealing/guardedValidation.property.spec.ts tests/suites/unit/framework/selfHealing/failureCapture.spec.ts tests/suites/unit/framework/selfHealing/registryPersistence.spec.ts --pool=threads --no-isolate --testTimeout=30000`; `rtk npm run test:unit`; `rtk npm run format:check`; `rtk npm run lint`; `rtk npm run typecheck`; `rtk npm run build` | Targeted PageObjectBase/pipeline unit specs passed (4 files / 25 tests); targeted self-healing telemetry helper specs passed (5 files / 31 tests); full unit suite passed 55 files / 348 tests; format, lint, typecheck, and build passed. Pipeline tests cover direct click/type delegation, guarded retry callbacks, original error cause preservation, single guarded retry call count on retry failure, and context-owned telemetry isolation from the singleton. | Continue with `AUR-IMPL-023` lifecycle implementation and fixture. |
| 2026-06-24 | Automation | `AUR-IMPL-024` | Read `AUR-DEC-013` and added a per-`AuroraFlowContext` self-healing run budget. `SELF_HEAL_RUN_BUDGET_MODE` defaults to `warning_only`; opt-in `enforce` downgrades failures beyond the healing-attempt budget to capture-only artifacts, then beyond the artifact budget to original-error-only. The failure path keeps legacy screenshots when self-healing is off, skips SAT/DOM/guarded validation/auto-apply/registry writes after the healing budget, emits one budget warning, and preserves the original page-action failure as the primary `PageActionError` cause. | `src/framework/selfHealing/{config,types,runBudget}.ts`, `src/framework/runtime/auroraFlowContext.ts`, `src/pageObjects/pageObjectBase.ts`, self-healing/page-object unit specs, `docs/configuration.md`, `docs/architecture/self-healing.md`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md` | `rtk npx vitest run tests/suites/unit/framework/selfHealing/config.spec.ts tests/suites/unit/framework/selfHealing/config.property.spec.ts tests/suites/unit/framework/pageObjectBase/pageObjectBaseSelfHealing.spec.ts --pool=threads --no-isolate --testTimeout=30000`; `rtk npx vitest run tests/suites/unit/framework/pageObjectBase/pageObjectBase.spec.ts --pool=threads --no-isolate --testTimeout=30000`; `rtk npm run test:unit`; `rtk npm run format:check`; `rtk npm run lint`; `rtk npm run typecheck`; `rtk npm run build`; `rtk npm run test:contracts`; `rtk npm run schemas:check` | Passed. Targeted budget/config/self-healing specs: 3 files / 38 tests. PageObjectBase regression spec: 1 file / 9 tests. Full unit suite: 55 files / 351 tests. Contracts: 19 files / 92 tests. Schemas compiled (10). Format, lint, typecheck, and build passed. Initial full unit run exposed legacy screenshot regression when self-healing was off; fixed by preserving screenshots outside the self-healing budget and reran the failing spec plus full unit. | Continue Phase 2 promotion governance (`AUR-IMPL-025`/`026`) or reconcile lifecycle fixture task status before starting the next PR. |
...
| 2026-06-10 | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts` | Run new parser regressions before hotfix | Failed as expected | Exposed apostrophe text parsing and escaped CSS unescaping gaps |
| 2026-06-10 | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts` | Validate minimal parser hotfix | Passed | Existing script ran 38 unit/framework files, 190 tests |
| 2026-06-10 | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts tests/suites/unit/framework/selfHealing/domCandidateExtraction.spec.ts` | Validate guarded parser plus DOM-emitted candidate compatibility | Passed | Existing script ran 38 unit/framework files, 191 tests |
| 2026-06-10 | `rtk npm run typecheck` | TypeScript strictness gate for parser/test changes | Passed | No errors |
| 2026-06-11 | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/historyRepository.spec.ts tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts tests/suites/unit/framework/selfHealing/domCandidateExtraction.spec.ts tests/suites/unit/framework/selfHealing/promotionWorkflow.spec.ts tests/suites/unit/framework/selfHealing/registryRuntime.spec.ts tests/suites/unit/framework/packageSurface/packageSurface.spec.ts` | Validate history TTL contract, 128-observation parallel-write counters, and touched store/test surfaces for `AUR-IMPL-005`/`006` | Passed | Existing script ran 39 unit/framework files, 199 tests |
| 2026-06-11 | `rtk npm run test:integration -- --run tests/suites/integration/framework/data/redisIntegration.spec.ts` | Validate Redis atomic JSON merge under 120 concurrent observations | Passed | 16 integration/contract files, 61 tests; zero lost increments against real Redis container |
| 2026-06-11 | `rtk npm run schemas:check` | Schema gate for candidate-history retention `$comment`/example | Passed | Compiled 10 JSON Schemas |
| 2026-06-11 | `rtk npm run typecheck` | TypeScript strictness gate for `AUR-IMPL-005`/`006` changes | Passed | No errors |
| 2026-06-11 | `rtk npm run lint` | ESLint gate | Passed | No errors |
| 2026-06-11 | `rtk npx prettier --write <5 touched files>` then `rtk npm run format:check` | Formatting gate | Passed | First check warned on five files Codex left unformatted; clean after targeted write |
| 2026-06-11 | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/config.spec.ts tests/suites/unit/framework/packageSurface/packageSurface.spec.ts` | Validate config diagnostics, strict mode, no-secret-echo, and package surface for `AUR-IMPL-007` | Passed | Existing script ran 39 unit/framework files, 212 tests |
| 2026-06-11 | `rtk npm run typecheck` | TypeScript strictness gate for `AUR-IMPL-007` | Passed | No errors |
| 2026-06-11 | `rtk npm run lint` and `rtk npm run format:check` | Lint and formatting gates for `AUR-IMPL-007` | Passed | format:check clean after one targeted Prettier write on the config spec |
| 2026-06-11 | `rtk npm run test:integration -- --run tests/suites/contracts/docs/documentationSurface.contract.spec.ts` | Docs contract after README/configuration diagnostics docs | Passed | 16 integration/contract files, 61 tests |
| 2026-06-11 | `rtk npm run schemas:check` | Schema gate for the Phase 0 review pass | Passed | Compiled 10 JSON Schemas |
| 2026-06-11 | `rtk npm run verify` | Full Phase 0 PR gate (format, lint, typecheck, unit, integration, shellcheck, workflows lint) | Passed | Unit 39 files / 213 tests; integration 16 files / 61 tests; one Prettier write needed first on the new legacy-store test |
| 2026-06-12 | `node -r ts-node/register <inventory dump>` via `tests/helpers/apiStabilitySurface.ts` | Generate exact root-export inventory for `AUR-IMPL-009` classification | Completed | 256 exports: 111 runtime, 145 type; manifest tables generated from the same parser the contract test uses |
| 2026-06-12 | `rtk npm run test:unit -- --run tests/suites/unit/framework/packageSurface/packageSurface.spec.ts tests/suites/unit/framework/publicApi/publicApi.spec.ts` | `AUR-IMPL-009` unit gate | Passed | Existing script ran 39 unit/framework files, 219 tests |
| 2026-06-12 | `rtk npm run test:integration -- --run tests/suites/contracts/package/packageSurface.contract.spec.ts` | `AUR-IMPL-009` contract gate | Passed | 16 integration/contract files, 65 tests |
| 2026-06-12 | Stale-row injection into `docs/api-stability.md` then contract rerun | Prove the stability contract fails on manifest drift | Failed as expected, then passed after restore | `NotARealExport` row flagged by 'lists no stale or mislabeled manifest entries' |
| 2026-06-12 | `rtk npm run typecheck`, `rtk npm run lint` | `AUR-IMPL-009` strictness gates | Passed | No errors |
| 2026-06-12 | `rtk npm run verify` | Full PR-01A gate | Passed | Unit 39 files / 219 tests; integration 16 files / 65 tests; format/lint/typecheck/shellcheck/workflows lint clean |
| 2026-06-12 | `rtk npm run workflows:lint` | `AUR-IMPL-010` actionlint gate on new `release.yml` | Passed | Native actionlint 1.7.11, no findings |
| 2026-06-12 | `rtk npm run workflows:security` | `AUR-IMPL-010` zizmor gate | Passed | zizmor 1.25.2: no findings across all five workflows including `release.yml` |
| 2026-06-12 | `rtk npm run test:integration -- --run tests/suites/contracts/workflows/release-dry-run.contract.spec.ts` | `AUR-IMPL-010` release workflow/doc contract gate | Passed after one fix | First run failed because the provenance note mentioned `npm publish` verbatim; reworded so the strict no-publish contract holds; rerun 17 files / 73 tests passed |
| 2026-06-12 | `rtk npm run build` and `rtk npm run pack:dry-run` | `AUR-IMPL-010` package dry-run evidence | Passed | `auroraflow-1.0.0.tgz` dry pack clean |
| 2026-06-12 | `npx npm@11 sbom --omit dev` (SPDX + CycloneDX) | Local SBOM smoke; local npm 9.2 predates `npm sbom` | Passed | 64 SPDX packages / 63 CycloneDX components; CI Node 22 toolchain bundles npm >= 10 so the workflow uses plain `npm sbom` |
| 2026-06-12 | Local execution of provenance-readiness and changelog-draft step logic | Validate inline workflow scripts before CI | Passed | Provenance prerequisites satisfied for `auroraflow@1.0.0`; changelog draft falls back to full history while no tag exists |
| 2026-06-12 | `rtk npm run verify` | Full PR-01B gate | Passed | Format/lint/typecheck/unit/integration/shellcheck/workflows lint clean with the new contract test included |
| 2026-06-13 | Test-first privacy unit/docs commands | Prove `AUR-IMPL-011`/`012` regression fixtures fail before implementation | Failed as expected | Missing privacy module and privacy-retention document were reported; one later analyzer assertion was narrowed to DOM-sourced candidates after correctly exposing existing heuristic candidates |
...
| 2026-06-18 | `rtk npm run test:unit -- --run tests/suites/unit/framework/selfHealing/candidateLocator.spec.ts tests/suites/unit/framework/selfHealing/guardedValidation.spec.ts tests/suites/unit/framework/selfHealing/artifactJsonSchemas.spec.ts tests/suites/unit/framework/selfHealing/artifactSchema.spec.ts tests/suites/unit/framework/selfHealing/domCandidateExtraction.spec.ts tests/suites/unit/framework/selfHealing/candidateScoring.spec.ts`; `rtk npm run schemas:check`; `rtk npm run typecheck`; `rtk npm run lint`; `rtk npm run format:check`; `rtk npm run test:integration -- --run tests/suites/contracts/package/packageSurface.contract.spec.ts` | `AUR-IMPL-020` targeted gates | Passed | Unit runner executed 50 files / 314 tests plus 1 expected fail; schemas compiled 10; typecheck/lint/format clean; package-surface contract passed 3 files / 23 tests |
| 2026-06-18 | `rtk npm run verify` | Full `AUR-IMPL-020` gate before taxonomy contract alignment | Failed, then fixed | Contract expected old risk-weighted coverage wording; test updated to the implemented `AUR-QE-109` docs wording through the existing compatibility assertion helper |
| 2026-06-18 | `rtk npm run test:contracts -- --run tests/suites/contracts/workflows/test-taxonomy.contract.spec.ts`; `rtk npm run verify` | Taxonomy rerun and final full `AUR-IMPL-020` gate | Passed | Contracts passed 19 files / 91 tests; full verify passed format/lint/typecheck, unit 50 files / 314 tests plus 1 expected fail, contracts 19 files / 91 tests, integration 2 files / 16 tests, schemas compiled 10, ShellCheck, and workflow lint; verify stamp skipped because worktree is intentionally dirty |
| 2026-06-18 | `rtk npm run test:unit -- --run tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts` | `AUR-IMPL-021` two-context isolation spec before migrating `PageObjectBase` internals | Failed as designed | Both isolation tests red (`expected [] to have a length of 1`): page-action telemetry routed to the module singleton because facade internals still read globals; confirmed the spec has teeth |
| 2026-06-18 | `rtk npm run typecheck`; `rtk npm run lint`; `rtk npm run format:check` | `AUR-IMPL-021` static gates after context migration | Passed | typecheck clean; eslint clean; format:check flagged 3 new/edited files, fixed via targeted `prettier --write` |
| 2026-06-18 | `rtk npm run test:unit -- --run tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts` | `AUR-IMPL-021` isolation spec after migrating internals | Passed | Two contexts isolate telemetry, self-healing mode, and run/test correlation with `SELF_HEAL_MODE`/`AURORAFLOW_RUN_ID` decoys ignored and no page-action telemetry on the singleton |
| 2026-06-18 | `rtk npm run test:unit` (full) | `AUR-IMPL-021` regression sweep incl. `pageObjectBase`/`pageFactory`/telemetry/publicApi/packageSurface | Passed | 51 files / 316 tests + 1 expected fail; earlier transient `workflowScriptsBoundary` flakiness-report child-process timeout did not recur |
| 2026-06-18 | `rtk npm run test:integration -- --run tests/suites/contracts/package/packageSurface.contract.spec.ts` | `AUR-IMPL-021` export/doc parity for 5 new runtime-context exports | Passed | 3 files / 23 tests; `src/index.ts` and `docs/api-stability.md` runtime-context rows agree |
| 2026-06-19 | `rtk npm run typecheck` | Verify provider-based `PageFactory` remediation restores TypeScript compatibility for `ExamplePage(page, url?)` | Passed | `PageObjectConstructor` is page-only; `getPage()` no longer requires constructor arg #2 to be `AuroraFlowContext` |
| 2026-06-19 | `rtk npm run test:unit -- --run tests/suites/unit/framework/pageFactory/pageFactory.spec.ts tests/suites/unit/framework/publicApi/publicApi.spec.ts tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts` | Attempt targeted remediation unit gate through npm script | Failed due command shape / unrelated transient | Script ran the whole unit suite and hit the known intermittent `workflowScriptsBoundary` child-process timeout; reran focused specs directly with Vitest |
| 2026-06-19 | `rtk npx vitest run tests/suites/unit/framework/pageFactory/pageFactory.spec.ts tests/suites/unit/framework/publicApi/publicApi.spec.ts tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts --pool=threads --no-isolate --testTimeout=30000` | Focused provider/compatibility unit gate | Passed | 3 files / 10 tests; covers legacy URL arg, no-explicit-constructor subclass, provider context injection, provider cache replacement, public provider type, and two-context provider isolation |
| 2026-06-19 | `rtk npm run test:contracts -- --run tests/suites/contracts/package/packageSurface.contract.spec.ts` | Package-surface docs/export contract after adding `PageObjectProvider` | Passed | Contract script ran all 19 contract files / 91 tests; root export inventory and `docs/api-stability.md` agree |
| 2026-06-19 | `rtk npm run format:check`; `rtk npx prettier --write ...`; `rtk npm run lint`; `rtk npm run format:check`; `rtk npm run typecheck` | Static gates after remediation docs/tests | Passed | Initial format check flagged `src/index.ts`; targeted Prettier made touched files clean; lint/typecheck passed |
| 2026-06-19 | `rtk npm run verify` | Full remediation gate | Passed | Unit 51 files / 320 tests + 1 expected fail; contracts 19 / 91; integration 2 / 16; schemas compiled 10; ShellCheck/workflow lint passed; verify stamp skipped because worktree is not clean |
| 2026-06-24 | Serena `get_symbols_overview` / `find_referencing_symbols`; `rtk rg` / `rtk sed` | `AUR-IMPL-027` prerequisite and API-impact inspection | Completed | Confirmed `AUR-IMPL-020` and `AUR-IMPL-022` completion evidence, `PageActionPipeline` click/type seam, and existing string-selector references. |
| 2026-06-24 | `rtk npm run format:check`; `rtk npx prettier --write docs/architecture/locator-first-api.md tests/suites/contracts/docs/documentationSurface.contract.spec.ts`; `rtk npm run format:check` | `AUR-IMPL-027` formatting gate | Failed, then passed | New design doc required Prettier formatting; rerun passed. |
| 2026-06-24 | `rtk npm run test:contracts -- --run tests/suites/contracts/docs/documentationSurface.contract.spec.ts` | Docs contract for locator-first compatibility and API-review gate | Failed, then passed | Initial run caught missing exact review-gate wording; explicit rule added. Rerun passed 19 files / 93 tests. |
| 2026-06-24 | `rtk npm run typecheck`; `rtk npm run lint` | Static gates after locator-first design contract | Passed | TypeScript and ESLint clean; no runtime API prototype added. |
...
Current handoff, 2026-06-18 Codex:

- **What was completed:** `AUR-IMPL-020` discriminated `CandidateLocator` model end-to-end: structured heuristic/DOM candidate emission, scoring propagation, guarded structured resolver, schema-versioned artifact shape, legacy string read converter, root export classification, and quote/legacy/schema regression coverage.
- **What remains:** Start `AUR-IMPL-021` (`AuroraFlowContext` runtime injection) before `AUR-IMPL-022`; `AUR-QE-112` can now build frame-aware structured candidate coverage on top of this model.
- **Current blockers:** None.
- **Validation status:** Full `npm run verify` passed on 2026-06-18: format, lint, typecheck, unit 50 files / 314 tests plus 1 expected fail, contracts 19 files / 91 tests, integration 2 files / 16 tests, schemas compiled 10, ShellCheck, workflow lint.
- **Files changed:** `src/framework/selfHealing/{candidateLocator,candidateTypes,domCandidateExtraction,suggestionEngine,candidateScoring,guardedValidation,types}.ts`, `src/index.ts`, `schemas/_shared.schema.json`, self-healing unit/schema tests, API/development/artifact-schema docs, and this implementation plan.
- **Important context:** Guarded validation now resolves structured candidates without parsing `locator` display strings; the legacy string path remains for old artifacts/arbitrary strings. Registry records still store display locators and convert through the legacy path until registry schema-version work (`AUR-IMPL-029`).
- **Recommended next task:** `AUR-IMPL-021`.
- **Rollback notes:** Revert structured locator source/tests/schema/docs while keeping the legacy parser path from `AUR-IMPL-003` if a hot rollback is needed.
- **Decision gates updated:** None; this implements `AUR-DEC-002`/API-tier-compatible Phase 2 work.
- **Journal entries added:** Current status snapshot, task progress, command log, and handoff notes updated for `AUR-IMPL-020`.

Current handoff, 2026-06-24 `AUR-IMPL-022`:

- **What was completed:** Added an internal `PageActionPipeline` and migrated `PageObjectBase.click()` / `type()` behind it. Public method signatures, timeout validation, success/error messages, guarded auto-heal result shape, and one guarded retry behavior are preserved. `PageActionError.originalError` remains intact and native `Error.cause` now points at the same original action failure.
- **Context threading:** The page-action failure path now passes the context-owned telemetry sink into suggestion generation, guarded validation, failure capture, and registry persistence. Omitted telemetry still falls back to the module singleton, preserving existing direct helper behavior.
- **What remains:** `AUR-IMPL-023` lifecycle implementation can bind the provider seam from the Playwright fixture and extend context lifecycle/disposables. Future pipeline slices should migrate `clickWhenVisible`, `read`, `wait`, screenshot, close, and navigation when they need behavior changes.
- **Current blockers:** None.
- **Validation status:** 2026-06-24 `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run build`, and full `npm run test:unit` passed (55 files / 348 tests). Targeted PageObjectBase/pipeline specs passed (4 files / 25 tests), and targeted self-healing telemetry helper specs passed (5 files / 31 tests).
- **Files changed:** `src/pageObjects/pageActionPipeline.ts`, `src/pageObjects/pageObjectBase.ts`, self-healing telemetry helper inputs, `tests/suites/unit/framework/pageObjectBase/pageActionPipeline.spec.ts`, `tests/suites/unit/framework/runtime/auroraFlowContext.isolation.spec.ts`, and this implementation plan.
- **Important context:** `createAuroraFlowContext()` with no options still reproduces global behavior. `PageFactory.getPage()` remains page-only; use `registerPageProvider()` or the future Playwright fixture when factory-owned context must participate in construction.
- **Recommended next task:** `AUR-IMPL-023`.
- **Rollback notes:** Revert `PageObjectBase.click()` / `type()` delegation to direct `safeAction` calls and remove `pageActionPipeline.ts` plus its tests. The telemetry input additions are additive and can remain or be reverted independently.
- **Decision gates updated:** None.
- **Journal entries added:** Current status snapshot, task progress, and handoff notes updated for `AUR-IMPL-022`.

Current handoff, 2026-06-24 `AUR-IMPL-024`:

- **What was completed:** Added a per-`AuroraFlowContext` self-healing run budget with env-backed config. Defaults are warning-only per `AUR-DEC-013`; opt-in enforcement supports capture-only then original-error-only downgrades.
- **Budget behavior:** `SELF_HEAL_RUN_BUDGET_MAX_HEALING_ATTEMPTS` bounds screenshots, SAT/DOM analysis, guarded validation, guarded auto-apply, and registry writes. `SELF_HEAL_RUN_BUDGET_MAX_FAILURE_ARTIFACTS` bounds artifact volume. A warning is emitted once per context after a configured max is exceeded.
- **Safety invariant:** Budget exhaustion never mutates selectors or source. The original page-action error remains `PageActionError.originalError`/`cause`; failed guarded retries stay secondary. Existing legacy screenshots are preserved when self-healing mode is `off`.
- **Validation status:** 2026-06-24 targeted budget/config/self-healing specs passed (3 files / 38 tests), PageObjectBase regression spec passed (1 file / 9 tests), full `npm run test:unit` passed (55 files / 351 tests), `npm run test:contracts` passed (19 files / 92 tests), `npm run schemas:check` passed (10 schemas), and `format:check`/`lint`/`typecheck`/`build` passed.
- **Files changed:** `src/framework/selfHealing/{config,types,runBudget}.ts`, `src/framework/runtime/auroraFlowContext.ts`, `src/pageObjects/pageObjectBase.ts`, self-healing/page-object tests, `docs/configuration.md`, `docs/architecture/self-healing.md`, and this implementation plan.
- **Recommended next task:** Completed by 2026-06-25 `AUR-IMPL-025`/`026`; continue with `AUR-IMPL-028` or reconcile `AUR-IMPL-023` lifecycle fixture status before sequencing.
- **Rollback notes:** Set `SELF_HEAL_RUN_BUDGET_MODE=warning_only` or revert `runBudget.ts` plus PageObjectBase budget gating. The config fields are additive, so warning-only mode is the safest compatibility fallback.

Current handoff, 2026-06-24 `AUR-IMPL-027`:

- **What was completed:** Design/API review package at `docs/architecture/locator-first-api.md` for additive Playwright `Locator` overloads on `PageObjectBase.click()` and `type()`, routed behind `PageActionPipeline`.
- **What remains:** API review. No overload prototypes, declaration changes, root exports, examples, or runtime behavior changes shipped in this task.
- **Current blockers:** Prototype work is intentionally blocked on API review acceptance.
- **Validation status:** 2026-06-24 `npm run format:check`, `npm run lint`, `npm run typecheck`, and targeted docs contract passed. Docs contract now runs 19 files / 93 tests.
- **Files changed:** `docs/architecture/locator-first-api.md`, `tests/suites/contracts/docs/documentationSurface.contract.spec.ts`, `docs/ARCHITECTURE_IMPLEMENTATION_PLAN.md`.
- **Important context:** String-selector APIs remain the stable public surface. The proposed locator path must not inspect Playwright private locator internals; it should use caller metadata (`targetAlias`, `selectorId`, `expectedRole`, `expectedName`) for telemetry/artifacts/registry context.
- **Recommended next task:** Submit API review for `AUR-IMPL-027`; otherwise continue `AUR-IMPL-028` Redis runbook/strategy or reconcile `AUR-IMPL-023` lifecycle fixture status.
- **Rollback notes:** Revert the locator-first design doc, docs contract additions, and this journal update. No runtime rollback needed.
- **Decision gates updated:** None.
- **Journal entries added:** Current status snapshot, task progress, command log, and handoff notes updated for `AUR-IMPL-027`.

Current handoff, 2026-06-25 `AUR-IMPL-025`/`AUR-IMPL-026`:

- **What was completed:** Promotion authorization policy, expected-status promotion CAS, and audit retention cleanup.
- **Authorization behavior:** Local mode is permissive and exposes a warning in workflow results/audit records/CLI stderr. Shared mode requires CODEOWNERS plus protected-workflow evidence before mutating selectors.
- **CAS behavior:** Promotion transitions use `SelectorStore.compareAndSetJsonField` with expected `status`, backed by Redis Lua and memory-store parity. Concurrent approve/reject/rollback races produce explicit `PromotionStatusConflictError` instead of last-writer-wins updates.
- **Retention behavior:** Audit writes get 30-day retention metadata. Cleanup scans history, promotion, and audit namespaces, defaults to dry-run, requires `--apply` or `SELF_HEAL_REGISTRY_CLEANUP_APPLY=true` for deletion, and skips `legalHold: true` audit records.
- **Validation status:** 2026-06-25 full `npm run verify` passed: format, lint, typecheck, unit (55 files / 358 tests), contracts (19 / 93), integration (2 / 16), schemas, ShellCheck, and workflow lint. `npm run build` and `npm run test:coverage:critical` also passed.
