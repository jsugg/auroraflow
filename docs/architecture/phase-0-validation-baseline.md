...
| --- | --- | --- | --- |
| `AUR-IMPL-001` | Decision log discoverability | `npm run format:check && npm run test:contracts -- tests/suites/contracts/docs/documentationSurface.contract.spec.ts` | Decision log contains `AUR-DEC-001` through `AUR-DEC-013`, owner, evidence, default, fallback, phase gate, affected tasks, and proceed/defer/non-goal authority. |
...
| `AUR-IMPL-004` | Scoring/SLO drift governance | `npm run test:unit -- --run tests/suites/unit/framework/selfHealing/candidateScoring.spec.ts tests/suites/unit/framework/observability/sloDashboard.spec.ts`; `npm run test:contracts -- tests/suites/contracts/workflows/slo-dashboard-alerting.contract.spec.ts` | Drift between scoring, dashboard, policy, and rules is either prevented or documented as intentional. |
...
| `AUR-IMPL-006` | Candidate-history TTL contract | `npm run test:unit -- --run tests/suites/unit/framework/selfHealing/historyRepository.spec.ts && npm run schemas:check` | Exported default, clamp, schema, and docs agree on TTL behavior. |
...
| `AUR-IMPL-008` | Baseline inventory | `npm run format:check && npm run test:contracts -- tests/suites/contracts/docs/documentationSurface.contract.spec.ts` | This baseline lists targeted commands, full commands, Redis/Docker fallback, and journal evidence format. |
