...
| `AUR-IMPL-004` | Scoring/SLO drift governance | `npm run test:unit -- --run tests/suites/unit/framework/selfHealing/candidateScoring.spec.ts tests/suites/unit/framework/observability/sloDashboard.spec.ts` | Drift between scoring, dashboard, policy, and rules is either prevented or documented as intentional. |
