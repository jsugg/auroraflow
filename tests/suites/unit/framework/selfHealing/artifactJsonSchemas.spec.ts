...
describe('self-healing artifact JSON Schemas', () => {
  it(
    'validates current DOM snapshot artifacts',
    async () => {
  ...
  it(
    'validates self-healing failure events with SAT and guarded summaries',
    async () => {
  ...
  it(
    'validates selector history and pending promotion contracts',
    async () => {
  ...
  it(
    'rejects malformed artifacts with actionable diagnostics',
    async () => {
      const validator = await validatorPromise;
